#include "pthread.h"
#include "stdio.h"
#include "stdlib.h"
#include "unistd.h"

#define TRUE 1

#define NE 2   // numero de escritores
#define NL 10  // numero de leitores

pthread_mutex_t lock_bd = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t lock_nl = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t lock_priority = PTHREAD_MUTEX_INITIALIZER;

int num_leitores = 0;
int num_escritores = 0;

void* reader(void* arg);
void* writer(void* arg);
void read_data_base();
void use_data_read();
void think_up_data();
void write_data_base();

int main() {
    pthread_t r[NL], w[NE];
    int i;
    int* id;
    /* criando leitores */
    for (i = 0; i < NL; i++) {
        id = (int*)malloc(sizeof(int));
        *id = i;
        pthread_create(&r[i], NULL, reader, (void*)(id));
    }
    /* criando escritores */
    for (i = 0; i < NE; i++) {
        id = (int*)malloc(sizeof(int));
        *id = i;
        pthread_create(&w[i], NULL, writer, (void*)(id));
    }
    pthread_join(r[0], NULL);
    return 0;
}

// Recebe um ponteiro para o id
void* reader(void* arg) {
    int i = *((int*)arg);
    while (TRUE) {
        pthread_mutex_lock(&lock_nl); 
        num_leitores++;
        if (num_leitores == 1) {
            pthread_mutex_lock(&lock_bd);  // Primeiro leitor bloqueia escritores
        }
        pthread_mutex_unlock(&lock_nl);  

        read_data_base(i); // É possível várias leituras simultâneas

        pthread_mutex_lock(&lock_nl);
        num_leitores--; // Sai um por vez
        if (num_leitores == 0) {
            pthread_mutex_unlock(&lock_bd);  // Último leitor libera escritores
        }
        pthread_mutex_unlock(&lock_nl);
        use_data_read(i);
        // Se for fazer usando um timer, eu crio uma variável e verifico se passou o tempo
        // Se sim, eu começo a travar as threads de leitores, até que um escritor escreva e libere os leitores dnv
        pthread_mutex_lock(&lock_priority);  // Dá prioridade para escritores -> Sempre que a operação de leitura é feita, ela vai esperar até que um escritor permita a próxima leitura
    }
    pthread_exit(0);
}

// Recebe um ponteiro para o id
void* writer(void* arg) {
    int i = *((int*)arg);
    while (TRUE) {
        think_up_data(i);
        pthread_mutex_lock(&lock_bd);  // Escritor bloqueia leitores e outros escritores
        num_escritores++;
        write_data_base(i);
        num_escritores--;
        pthread_mutex_unlock(&lock_bd);
        pthread_mutex_unlock(&lock_priority);  // Permite uma leitura após a escrita
    }
    pthread_exit(0);
}

void read_data_base(int i) {
    printf("Leitor   %d está lendo os dados! Número de leitores: %d. Número de escritores: %d\n", i, num_leitores, num_escritores);
    sleep(rand() % 5);
}

void use_data_read(int i) {
    printf("Leitor %d está usando os dados lidos! Número de leitores: %d\n", i, num_leitores);
    sleep(rand() % 5);
}

void think_up_data(int i) {
    // printf("Escritor %d está pensando no que escrever!\n", i);
    sleep(rand() % 5);
}

void write_data_base(int i) {
    printf("Escritor %d está escrevendo os dados! Número de leitores: %d\n", i, num_leitores);
    sleep(rand() % 5 + 5);
}
