...
void* macacoAB(void* a) {
    ...
    while (1) {
        while (1) {                      // Loop de espera para entrar na corda
            pthread_mutex_lock(&corda);  // Mesmo que a thread pegue esse lock antes de gorila, se "gorila==1", ele vai automaticamente liberar -> provável starvation?
            if (BA == 0 && gorila_esperando == 0) {
                AB++;  // Aumenta o contador, faz as demais threads entrar no loop de espera
        ...
        pthread_mutex_lock(&turno);      // Garante que apenas um por vez passa daqui
        pthread_mutex_lock(&mutexAB);    // Protege o contador e garante que apenas um AB COMECE a atravessar por vez -> vários na corda, mas entram um por vez
        if (AB == 1) {                   // Se AB > 1, significa que um macaco AB ESTÁ na corda. Logo a direção "A -> B" está livre. Logo macaco AB pode começar a passagem
            pthread_mutex_lock(&corda);  // Garante a direção de passagem -> Suponha que o primeiro AB passou de "turno" e que "corda" está travado,
                                         // essa thread vai ficar presa aqui. Os próximos ABs não vão passar por conta do lock protegendo o contador.
        }
        pthread_mutex_unlock(&mutexAB);  // Libera para o próximo macaco AB
        pthread_mutex_unlock(&turno);    // Libera para qualquer outro macaco
        ...
        if (AB == 0) {
            pthread_mutex_unlock(&corda);  // Libera corda, assim a direção pode ser alterada pela próxima thread
        }
...
void* macacoBA(void* a) {
    ...
    while (1) {
        ...
        while (1) {                      // Loop de espera para entrar na corda
            pthread_mutex_lock(&corda);  // Mesmo que a thread pegue esse lock antes de gorila, se "gorila==1", ele vai automaticamente liberar -> provável starvation?
            if (AB == 0 && gorila_esperando == 0) {
                BA++;  // Aumenta o contador, faz as demais threads entrar no loop de espera
                pthread_mutex_unlock(&corda);
                break;  // Sai do loop
            }
            pthread_mutex_unlock(&corda);
