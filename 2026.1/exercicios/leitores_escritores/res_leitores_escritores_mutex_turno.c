...
int main() {
    pthread_mutex_init(&mutex, NULL);  // Exclusão mútua contador de leitores (rc)
    pthread_mutex_init(&db, NULL);     // Exclusão mútua para escrita
    pthread_mutex_init(&turno, NULL);  // Exclusão mútua escrita e leitura
